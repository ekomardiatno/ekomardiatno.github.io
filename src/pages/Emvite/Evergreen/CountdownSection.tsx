...
function TimeBox({ value, label }: { value: number; label: string }) {
  return (
    <div className="flex flex-col items-center rounded-xl bg-white border border-emerald-200 px-5 py-4 shadow-sm flex-[85px] grow-0">
      <span className="text-2xl font-semibold text-emerald-800">
        {value.toString().padStart(2, "0")}
      </span>
      <span className="mt-1 text-xs uppercase tracking-wide text-stone-500">
        {label}
      </span>
    </div>
  );
...
export default function CountdownSection() {
  ...
  return (
    <section id="countdown" className="px-6 py-20 text-center">
      <div className="mb-12">
        <h2 className="mb-4 font-serif text-3xl text-emerald-800">
