...
export default function GuestbookSection() {
  ...
  return (
    <section id="guestbook" className="bg-white px-6 py-20 text-stone-800">
      <div className="mb-12 text-center">
        <h2 className="mb-4 font-serif text-3xl text-emerald-800">Ucapan & Doa</h2>
      ...
      <div className="mx-auto grid max-w-5xl gap-12 md:grid-cols-2">
        <form onSubmit={submitWish} className="space-y-6">
          <div>
            ...
            <input
              type="text"
              className="w-full rounded-lg border border-stone-300 px-4 py-3 text-sm focus:border-emerald-600 focus:outline-none"
              value={name}
          ...
          <button
            type="submit"
            className="w-full rounded-full bg-emerald-800 px-6 py-4 text-sm tracking-wide text-white transition hover:bg-emerald-700 cursor-pointer disabled:cursor-not-allowed"
        ...
        <div className="max-h-105 space-y-4 overflow-y-auto pr-2">
          ...
          {data.wishes.map((wish, index) => (
            <div
              key={index}
              className="rounded-2xl border border-emerald-100 bg-stone-50 p-4"
            >
              {/* Header */}
